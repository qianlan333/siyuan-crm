...
class SqlAlchemyWorkflowNodeRepository(InMemoryAutomationRepository):
    """Explicit test/staging DB adapter for workflow-node metadata only."""

    source_status = "sql_alchemy_workflow_node_repository"
    ...
    def list_workflow_nodes(self, filters: dict[str, Any] | None = None) -> tuple[list[dict[str, Any]], int]:
        ...
        try:
            with self._engine.connect() as conn:
                ...
                rows = conn.execute(
                    text(
                        ...
                        {where}
                        ORDER BY workflow_id ASC, sort_order ASC, id ASC
            ...
            raise RepositoryProviderError(f"workflow node repository unavailable: {exc}") from exc
    ...
    def create_workflow_node(self, payload: dict[str, Any], *, idempotency_key: str, operator: str) -> dict[str, Any]:
        ...
        try:
            ...
            raise ContractError("workflow node code already exists for workflow") from exc
        except SQLAlchemyError as exc:
            raise RepositoryProviderError(f"workflow node repository unavailable: {exc}") from exc
            ...
            with self._engine.connect() as conn:
                rows = conn.execute(
                    text(
                        """
                        SELECT *
                        FROM automation_workflow_node_audit_log
    ...
    def _load_idempotency(self, conn: Any, *, operator: str, idempotency_key: str) -> dict[str, Any] | None:
        row = conn.execute(
            text(
                """
                SELECT *
                FROM automation_workflow_node_idempotency
                WHERE route_family = :route_family
    ...
    def _insert_idempotency_pending(self, conn: Any, *, operator: str, idempotency_key: str, request_hash: str) -> None:
        conn.execute(
            text(
                """
                INSERT INTO automation_workflow_node_idempotency (
                ...
                VALUES (
                    ...
                    :response_snapshot, 'workflow_node', NULL, 'pending', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
                )
                """
            ),
            {
    ...
    def _mark_idempotency_succeeded(
        ...
        *,
        ...
        conn.execute(
            text(
                """
                UPDATE automation_workflow_node_idempotency
    ...
    def _assert_unique_workflow_node_sql(self, conn: Any, normalized: dict[str, Any]) -> None:
        row = conn.execute(
            text(
                """
                SELECT id
                ...
                WHERE workflow_id = :workflow_id
                  AND LOWER(node_code) = LOWER(:node_code)
                LIMIT 1
                """
            ),
            {
                "workflow_id": int(normalized["workflow_id"]),
                "node_code": normalized["node_code"],
            },
        ).fetchone()
        if row is not None:
            raise ContractError("workflow node code already exists for workflow")

    def _insert_workflow_node(self, conn: Any, normalized: dict[str, Any]) -> dict[str, Any]:
        params = {
            ...
            "position_json": _json_dumps(normalized.get("position") or {}),
        ...
        result = conn.execute(
            text(
                ...
                INSERT INTO automation_workflow_nodes (
                    program_id, workflow_id, node_code, node_name, node_type, status,
                    sort_order, position_json, metadata_json, config_json, enabled,
        ...
        if not inserted_id:
            inserted_id = int(conn.execute(text("SELECT MAX(id) FROM automation_workflow_nodes")).scalar_one() or 0)
        row = conn.execute(text("SELECT * FROM automation_workflow_nodes WHERE id = :id"), {"id": inserted_id}).fetchone()
    ...
    def _insert_audit_event(
        ...
        *,
        ...
        conn.execute(
            text(
                """
                INSERT INTO automation_workflow_node_audit_log (
                ...
                VALUES (
                    :route_family, :operation, :operator, 'workflow_node', :resource_id,
                    :before_snapshot, :after_snapshot, :request_payload, :validation_result,
            ...
            ),
            {
                ...
                "side_effect_safety": _json_dumps(workflow_node_side_effect_safety()),
            },
        )
        row = conn.execute(
            text("SELECT * FROM automation_workflow_node_audit_log WHERE id = (SELECT MAX(id) FROM automation_workflow_node_audit_log)")
        ).fetchone()
        return self._audit_row_to_event(_as_mapping(row) or {})

    def _row_to_projection(self, row: dict[str, Any]) -> dict[str, Any]:
        return workflow_node_projection(
            {
                "id": row.get("id"),
                ...
                "position": _json_loads(row.get("position_json"), {}),
    ...
    def _audit_row_to_event(self, row: dict[str, Any]) -> dict[str, Any]:
        return {
            "route_family": row.get("route_family") or WORKFLOW_NODE_ROUTE_FAMILY,
            "operation": row.get("operation") or "",
            "operator": row.get("operator") or "",
            "resource_type": row.get("resource_type") or "workflow_node",
            ...
            "side_effect_safety": _json_loads(row.get("side_effect_safety"), workflow_node_side_effect_safety()),
            "created_at": str(row.get("created_at") or utc_now_iso()),
