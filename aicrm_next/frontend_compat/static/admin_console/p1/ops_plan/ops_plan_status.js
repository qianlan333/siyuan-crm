...
export function normalizeOpsPlanStatus(rawStatus, options = {}) {
...
export function toOpsPlanScenario(input) {
    return {
...
export function opsPlanGuardrailsForScenario(scenario) {
...
export function buildOpsPlanStatusViewModel(input) {
...
export function opsPlanPromptForStatus(status) {
    if (status === "downstream-pending")
        return "Planner 已到 broadcast_job / Push Center pending；下游 external effect 尚未执行，不能显示为 completed。";
    if (status === "pending")
        return "事件或 projection 仍待处理，不能显示为 sent。";
    if (status === "sent")
        return "仅当 external effect sent evidence 成立时才可显示为 sent。";
    ...
    if (status === "evidence-incomplete")
        return "缺少下游 evidence，不能渲染为 pass-complete。";
    return "只读展示，不绑定审批、任务执行或外呼。";
}
export function pushCenterPendingIsSent(input) {
...
export function defaultOpsPlanViewModels() {
