...
export function normalizeWeComStatus(rawStatus, options = {}) {
...
export function toWeComScenario(input) {
    return {
        key: "wecom_auth",
        title: input.title,
...
export function weComGuardrailsForScenario(scenario) {
...
export function buildWeComStatusViewModel(input) {
...
export function weComPromptForStatus(status) {
    if (status === "external-config-blocked")
        return "外部配置未批准生效；不能显示为授权完成，也不能进入可执行队列。";
    if (status === "blocked")
        return "Callback fail-closed 是安全阻塞，不是回调成功。";
    if (status === "evidence-incomplete")
        return "缺 valid callback signature / auth record / internal_event / permission scope evidence，不能显示为 complete。";
    if (status === "operator-action-required")
        return "需要 operator 完成 git 外配置和 evidence window。";
    if (status === "failed-terminal")
        return "终态失败，不能自动重试。";
    if (status === "pending")
        return "仍待配置或 evidence，不代表授权完成。";
    return "只读展示，不绑定真实授权、callback、配置写入或外呼。";
}
export function externalConfigBlockedIsAuthorized(input) {
...
export function defaultWeComViewModels() {
