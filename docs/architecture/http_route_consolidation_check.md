...
| Area | Current state |
| --- | --- |
| Route module registry | `HTTP_ROUTE_MODULES` contains 71 route-owner or child-controller modules |
| Direct registrars | `HTTP_ROUTE_REGISTRARS` contains 47 modules registered directly on the Flask blueprint |
...
| Helper modules | 15 unregistered HTTP modules are explicitly listed as helper-only modules in `tests/test_http_registration_contract.py` |
| Exported route inventory | `scripts/export_flask_routes.py` exported 415 route rows in this snapshot |
...
| Route export | `python3 scripts/export_flask_routes.py > /tmp/ai_crm_routes.json && jq -r '.route_count' /tmp/ai_crm_routes.json` | verifies the generated inventory remains exportable |
...
| `wecom_ability_service/http/automation_conversion.py` | 308 | route aggregator; acceptable while it remains registration-only |
...
| `wecom_ability_service/http/wechat_pay.py` | 226 | acceptable for now; keep payment client work in the domain |
...
| `wecom_ability_service/http/automation_conversion_page_actions.py` | 213 | agent orchestration and auto-reply monitor page actions have been split into focused child controllers |
...
| `wecom_ability_service/http/automation_conversion_workspaces.py` | 577 | page payload assembly; better next target is domain/application payload ownership, not more HTTP files |
...
| `wecom_ability_service/http/automation_conversion_render.py` | 340 | render adapter helpers; keep out of the route aggregator |
