...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `/api/admin/class-user-management/export` | `aicrm_next/frontend_compat/templates/admin_console/operations.html` export button | GET download; POST/OPTIONS supported by API closeout | Frontend button exists and previously pointed at an unregistered deferred export URL | Next-owned controlled local CSV export | `next_export` / `aicrm_next.post_legacy_deferred` | none; `external_storage_executed=false` | GET must return 200 CSV, `route_owner=ai_crm_next`, `fallback_used=false` | Keeps the UI aligned by making the existing export button resolve without storage writes. |
