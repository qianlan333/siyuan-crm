...
- Response includes `ok`, `command_id`, `submission_id`, `questionnaire_id`, `slug`, `result`, `external_push`, `tag_apply`, `internal_event_id`/`internal_event_outbox_id`, `durable_continuation_queued`, `source_status=next_command`, `route_owner=ai_crm_next`, `fallback_used=false`, and `real_external_call_executed=false`.
