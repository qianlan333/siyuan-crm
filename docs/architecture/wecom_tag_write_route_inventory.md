...
`aicrm_next/customer_tags/commands.py` defines the write command shapes. `aicrm_next/customer_tags/admin_write.py` owns CommandBus dispatch, validation, idempotency, audit recording, production blocking, opt-in live CRUD execution, and response shape. `aicrm_next/customer_tags/write_repo.py` owns `WeComTagWriteRepository`, the local projection write fixture repository.

By default, successful fixture command responses must include `route_owner=ai_crm_next`, `source_status=next_command`, `fallback_used=false`, `real_external_call_executed=false`, `local_only=true`, and a `side_effect_plan` with `adapter_mode=real_blocked`.

Production CRUD is available only when production data is ready and `AICRM_WECOM_TAG_CRUD_LIVE_ENABLED=1`. In that mode the command response must keep `route_owner=ai_crm_next` and `fallback_used=false`, set `write_model_status=live_wecom_executed`, `real_external_call_executed=true`, `local_only=false`, `sync_executed=true`, and record an executed `side_effect_plan` with `adapter_mode=real_live`. The command then refreshes the tag catalog projection through the Next live sync path.
...
Real WeCom create/update/delete is not executed by default. The command layer records a `SideEffectPlan`, keeps `adapter_mode=real_blocked`, and production data mode returns `production_unavailable` until `AICRM_WECOM_TAG_CRUD_LIVE_ENABLED=1` is explicitly configured. With that switch enabled, exact CRUD routes call the Next `WeComTagLiveGateway` for `add_corp_tag`, `edit_corp_tag`, or `del_corp_tag`, then refresh the local catalog projection.
