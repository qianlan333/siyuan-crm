# siyuan AI-CRM Next WeCom Auth Readiness After Env Enablement - 2026-06-09
...
- 代码 commit：`ea0192b6`。
- PR #56：已包含。
- GitHub 拉取说明：本次服务器访问 `github.com:443` 出现超时；未继续依赖远端 fetch。当前服务器 worktree 已在 PR #56 merge commit `ea0192b6`，满足本次 auth readiness 验证要求。
- 临时 readiness 服务：使用 `127.0.0.1:5101`，避免占用当前生产 systemd 服务监听的 `127.0.0.1:5001`。
- 本次未切生产。
- 本次未修改 systemd/nginx。
...
- 临时本机 HTTP readiness 服务使用 `AICRM_NEXT_ADMIN_SESSION_COOKIE_SECURE=false` 覆盖，仅用于本机 smoke；正式 HTTPS 生产访问仍应使用 secure cookie 策略。
...
| Open redirect defense | pass | HTTP 302 to WeCom QR authorize URL; unsafe external next path is not surfaced in the response body or report |

Result:

- `external_call_blocked` no longer appears in the auth smoke responses.
