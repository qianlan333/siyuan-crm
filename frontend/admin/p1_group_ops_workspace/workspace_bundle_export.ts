...
function redactCopyValue(value: unknown): string {
...
function safeItemSummary(item: WorkspacePreviewBundleItem): CopySafeBundleItemSummary {
  return {
...
function guardrailSummary(bundle: WorkspacePreviewBundle): string[] {
  return [
...
function buildCopySafeObject(
  bundle: WorkspacePreviewBundle,
  context: CopySafeBundleContext = {}
): CopySafeBundleJson {
  return {
...
export function assertCopySafeBundleOutput(output: string): true {
...
export function buildCopySafeBundleJson(
  bundle: WorkspacePreviewBundle,
  context: CopySafeBundleContext = {}
): string {
...
export function buildCopySafeBundleText(
  bundle: WorkspacePreviewBundle,
  context: CopySafeBundleContext = {}
): string {
  ...
  const lines = [
    ...
    ...payload.selectedItems.map((item) => (
      `- ${item.entityType} ${item.detailId}: ${item.title} | ${item.status} | ${item.derivedStatus} | visible=${item.visibleInCurrentFilter}`
    ))
  ];
...
export async function copyBundleSummaryToClipboard(
  output: string,
  clipboard: CopySafeClipboard | undefined = typeof navigator !== "undefined" ? navigator.clipboard : undefined
): Promise<CopySafeResult> {
  try {
    assertCopySafeBundleOutput(output);
