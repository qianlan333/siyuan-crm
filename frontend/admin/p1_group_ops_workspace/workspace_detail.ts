...
import { type ScenarioEvidence } from "../shared/status_model.js";
import {
...
export function createWorkspaceSelectionState(fixture: WorkspaceFixture): WorkspaceSelectionState {
  return { ...fixture.defaultSelection };
}

export function selectedDetailId(selection: WorkspaceSelectionState): string {
...
export function findWorkspaceDetail(
...
export function selectWorkspaceEntity(
  current: WorkspaceSelectionState,
  entityType: WorkspaceEntityType,
  detailId: string
): WorkspaceSelectionState {
  const next = { ...current, selectedEntityType: entityType };
  if (entityType === "plan") next.selectedPlanId = detailId;
  if (entityType === "group") next.selectedGroupId = detailId;
  if (entityType === "node") next.selectedNodeId = detailId;
  if (entityType === "execution") next.selectedExecutionId = detailId;
  if (entityType === "push_center") next.selectedPushCenterJobId = detailId;
  return next;
}

function scenarioFromDetail(detail: WorkspaceDetailItem): ScenarioEvidence {
  return {
...
function renderDetailFields(detail: WorkspaceDetailItem): string {
...
export function renderWorkspaceDetailPanel(fixture: WorkspaceFixture, selection: WorkspaceSelectionState): string {
...
export function renderWorkspaceSelectedPreviewResult(
  fixture: WorkspaceFixture,
  selection: WorkspaceSelectionState
): string {
  const detail = findWorkspaceDetail(fixture, selection);
