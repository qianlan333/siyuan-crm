...
import { type WorkspaceDetailItem, type WorkspaceEntityType, type WorkspaceFixture } from "./workspace_fixture.js";
...
function emptyCounts(): Record<WorkspaceEntityType, number> {
  return {
    plan: 0,
    group: 0,
    node: 0,
...
function isBlocked(status: EvidenceStatus): boolean {
...
function needsAction(status: EvidenceStatus): boolean {
...
function detailLookup(fixture: WorkspaceFixture, entityType: WorkspaceEntityType, detailId: string): WorkspaceDetailItem | undefined {
  return fixture.detailItems.find((item) => item.entityType === entityType && item.id === detailId);
}

export function buildWorkspacePreviewBundle(
  ...
  const items: WorkspacePreviewBundleItem[] = [];
