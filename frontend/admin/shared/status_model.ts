...
const NORMALIZED_STATUS: Record<string, EvidenceStatus> = {
  ready: "ready",
  pending: "pending",
  sent: "sent",
  blocked: "blocked",
...
export function normalizeEvidenceStatus(rawStatus: string): EvidenceStatus {
  return NORMALIZED_STATUS[rawStatus] ?? "evidence-incomplete";
}

export function statusMeta(status: EvidenceStatus): StatusMeta {
  return STATUS_META[status];
}

export function canRenderGlobalPass(payload: BusinessClosurePayload): boolean {
...
export function scenarioNeedsOperatorAction(scenario: ScenarioEvidence): boolean {
