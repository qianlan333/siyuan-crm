...
- ``tags`` (JSONB / TEXT json)：自由标签数组，如 ``["好评截图","信任建立"]``
- ``category`` (TEXT)：单值大类，如 ``"好评截图"`` / ``"产品案例"``
- ``ai_metadata`` (JSONB / TEXT json)：AI 分析的结构化扩展信息，schema 不约束
...
def upgrade() -> None:
    if _is_postgres():
        ...
        _add_column("image_library", "description TEXT NOT NULL DEFAULT ''", "description")
        _add_column("image_library", "tags TEXT NOT NULL DEFAULT '[]'", "tags")
        _add_column("image_library", "category TEXT NOT NULL DEFAULT ''", "category")
        _add_column("image_library", "ai_metadata TEXT NOT NULL DEFAULT '{}'", "ai_metadata")
        op.execute(
            "CREATE INDEX IF NOT EXISTS idx_image_library_category "
