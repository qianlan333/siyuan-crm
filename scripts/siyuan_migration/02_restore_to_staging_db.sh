#!/usr/bin/env bash
set -euo pipefail

DUMP_FILE="${DUMP_FILE:-}"
STAGING_DATABASE_URL="${STAGING_DATABASE_URL:-}"
CLEAN="${CLEAN:-true}"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

pass() { printf 'PASS %s\n' "$*"; }
fail() { printf 'FAIL %s\n' "$*"; exit 1; }

# shellcheck disable=SC1091
source "${SCRIPT_DIR}/lib_db_url.sh"

[[ -n "${DUMP_FILE}" ]] || fail "DUMP_FILE is required"
[[ -f "${DUMP_FILE}" ]] || fail "DUMP_FILE not found: ${DUMP_FILE}"
[[ -n "${STAGING_DATABASE_URL}" ]] || fail "STAGING_DATABASE_URL is required and must be explicit"
if ! PG_CLI_STAGING_DATABASE_URL="$(normalize_pg_cli_url "${STAGING_DATABASE_URL}")"; then
  fail "STAGING_DATABASE_URL must use postgresql://, postgres://, postgresql+psycopg://, or postgresql+psycopg2://"
fi
pass "STAGING_DATABASE_URL is available for PostgreSQL CLI tools"

if [[ "${CLEAN}" == "true" ]]; then
  pg_restore --clean --if-exists --no-owner --dbname="${PG_CLI_STAGING_DATABASE_URL}" "${DUMP_FILE}"
else
  pg_restore --no-owner --dbname="${PG_CLI_STAGING_DATABASE_URL}" "${DUMP_FILE}"
fi

pass "restored ${DUMP_FILE} to explicit staging database"
