...
def test_automation_application_skeleton_delegates_to_legacy_module(monkeypatch):
    calls: dict[str, object] = {}
    ...
    monkeypatch.setattr(
        "wecom_ability_service.application.automation_engine._legacy_delegate.get_signup_conversion_config_legacy",
        _record("get_signup_conversion_config", {"ok": True}),
    )
    monkeypatch.setattr(
        "wecom_ability_service.application.automation_engine._legacy_delegate.save_signup_conversion_config_legacy",
        _record("save_signup_conversion_config", {"ok": True}),
    )
    monkeypatch.setattr(
        "wecom_ability_service.application.automation_engine._legacy_delegate.preview_signup_conversion_customer_legacy",
        _record("preview_signup_conversion_customer", {"ok": True}),
    )
    monkeypatch.setattr(
        "wecom_ability_service.application.automation_engine._legacy_delegate.recompute_signup_conversion_customers_legacy",
        _record("recompute_signup_conversion_customers", {"ok": True}),
    )
    monkeypatch.setattr(
        "wecom_ability_service.application.automation_engine._legacy_delegate.list_signup_conversion_batches_legacy",
        _record("list_signup_conversion_batches", {"ok": True}),
    )
    monkeypatch.setattr(
        "wecom_ability_service.application.automation_engine._legacy_delegate.get_signup_conversion_batch_legacy",
        _record("get_signup_conversion_batch", {"ok": True}),
    )
    monkeypatch.setattr(
        "wecom_ability_service.application.automation_engine._legacy_delegate.get_outbound_webhook_delivery_counts_legacy",
        lambda: {"failed": 1},
    )
    monkeypatch.setattr(
        "wecom_ability_service.application.automation_engine._legacy_delegate.list_outbound_webhook_deliveries_legacy",
        _record("list_outbound_webhook_deliveries", {"ok": True}),
    )
    monkeypatch.setattr(
        "wecom_ability_service.application.automation_engine._legacy_delegate.retry_outbound_webhook_delivery_legacy",
        _record("retry_outbound_webhook_delivery", {"ok": True}),
    )
    monkeypatch.setattr(
        "wecom_ability_service.application.automation_engine._legacy_delegate.run_due_outbound_webhook_retries_legacy",
        _record("run_due_outbound_webhook_retries", {"ok": True}),
    )
    monkeypatch.setattr(
        "wecom_ability_service.application.automation_engine._legacy_delegate.sync_automation_member_activation_legacy",
        _record("sync_automation_member_activation", {"ok": True}),
    )
    monkeypatch.setattr(
        "wecom_ability_service.application.automation_engine._legacy_delegate.apply_activation_webhook_legacy",
        _record("apply_activation_webhook", {"customer": {"external_userid": "wm_ext_001"}}),
    )
    monkeypatch.setattr(
        "wecom_ability_service.application.automation_engine._legacy_delegate.record_conversion_feedback_legacy",
        _record("record_conversion_feedback", {"ok": True}),
    )
    monkeypatch.setattr(
        "wecom_ability_service.application.automation_engine._legacy_delegate.acknowledge_conversion_batch_legacy",
        _record("ack_conversion_batch", {"ok": True}),
    )
    monkeypatch.setattr(
        "wecom_ability_service.application.automation_engine._legacy_delegate.get_customer_marketing_profile_legacy",
        _record("get_customer_marketing_profile", {"ok": True}),
    )
    monkeypatch.setattr(
        "wecom_ability_service.application.automation_engine._legacy_delegate.mark_enrolled_legacy",
        _record("mark_enrolled", {"ok": True}),
    )
    monkeypatch.setattr(
        "wecom_ability_service.application.automation_engine._legacy_delegate.unmark_enrolled_legacy",
        _record("unmark_enrolled", {"ok": True}),
    )
    monkeypatch.setattr(
        "wecom_ability_service.application.automation_engine._legacy_delegate.set_manual_followup_segment_legacy",
        _record("set_manual_followup_segment", {"ok": True}),
    )

    assert automation_queries.GetSignupConversionConfigQuery()(SignupConversionConfigQueryDTO()) == {"ok": True}
    assert automation_commands.SaveSignupConversionConfigCommand()(
        SignupConversionConfigCommandDTO(payload={"enabled": True})
    ) == {"ok": True}
    assert automation_queries.PreviewSignupConversionCustomerQuery()(
        SignupConversionPreviewQueryDTO(external_userid="wm_ext_001")
    ) == {"ok": True}
    assert automation_commands.RecomputeSignupConversionCustomersCommand()(
        SignupConversionRecomputeCommandDTO(external_userid="wm_ext_001")
    ) == {"ok": True}
    assert automation_queries.ListSignupConversionBatchesQuery()(SignupConversionBatchListQueryDTO()) == {"ok": True}
    assert automation_queries.GetSignupConversionBatchQuery()(
        SignupConversionBatchDetailQueryDTO(batch_id=9)
    ) == {"ok": True}
    assert automation_queries.GetOutboundWebhookDeliveryCountsQuery()(OutboundWebhookCountQueryDTO()) == {"failed": 1}
    assert automation_queries.ListOutboundWebhookDeliveriesQuery()(OutboundWebhookListQueryDTO()) == {"ok": True}
    assert automation_commands.RetryOutboundWebhookDeliveryCommand()(OutboundWebhookRetryCommandDTO(delivery_id=7)) == {
        "ok": True
    }
    assert automation_commands.RunDueOutboundWebhookRetriesCommand()(
        OutboundWebhookRetryBatchCommandDTO(limit=3)
    ) == {"ok": True}
    assert automation_commands.SyncAutomationMemberActivationCommand()(
        external_contact_id="wm_ext_001"
    ) == {"ok": True}
    assert automation_commands.ApplyActivationWebhookCommand()(mobile="13800138000") == {
        "customer": {"external_userid": "wm_ext_001"}
    }
    assert automation_commands.RecordConversionFeedbackCommand()(
        ConversionFeedbackCommandDTO(feedback_type="manual_note")
    ) == {"ok": True}
    assert automation_commands.AcknowledgeConversionBatchCommand()(
        ConversionBatchAckCommandDTO(batch_id=5)
    ) == {"ok": True}
    assert automation_queries.GetCustomerMarketingProfileQuery()(
        CustomerMarketingProfileQueryDTO(external_userid="wm_ext_001")
    ) == {"ok": True}
    assert automation_commands.MarkEnrolledCommand()(MarkEnrolledCommandDTO(external_userid="wm_ext_001")) == {
        "ok": True
    }
    assert automation_commands.UnmarkEnrolledCommand()(UnmarkEnrolledCommandDTO(external_userid="wm_ext_001")) == {
        "ok": True
    }
    assert automation_commands.SetManualFollowupSegmentCommand()(
        ManualFollowupSegmentCommandDTO(external_userid="wm_ext_001", followup_segment="focus")
    ) == {"ok": True}
