...
def test_removed_legacy_startup_commands_fail_without_flask(argv: list[str], command: str) -> None:
    ...
    assert "has been removed. AI-CRM now starts with Next runtime only." in message
    assert "python app.py run" in message
    assert "For database schema changes use Alembic migrations." in message
