...
def test_user_ops_customer_drawer_and_timeline_are_next_native() -> None:
    ...
    detail_response = client.get("/api/admin/user-ops/customers/union_ops_001")
    timeline_response = client.get("/api/admin/user-ops/customers/union_ops_001/timeline?limit=10")

    assert detail_response.status_code == 200
    detail = detail_response.json()
    assert detail["route_owner"] == "ai_crm_next"
    assert detail["fallback_used"] is False
    assert detail["customer"]["unionid"] == "union_ops_001"
    ...
    assert timeline["unionid"] == "union_ops_001"
